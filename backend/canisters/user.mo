...
persistent actor UserCanister {
    ...
    // Storage
    private var userEntries: [(UserId, User)] = [];
    private var emailToUserIdEntries: [(Email, UserId)] = [];
    private var otpEntries: [(Email, OTPData)] = [];

    // Flexible storage that doesn't persist across upgrades
    ...
    system func preupgrade() {
        ...
        emailToUserIdEntries := Iter.toArray(emailToUserId.entries());
        otpEntries := Iter.toArray(otpStore.entries());
    };

    // Migration function to handle type changes
    private func migrateUser(user: User): User {
        {
            id = user.id;
            email = user.email;
    ...
    system func postupgrade() {
        for ((id, user) in userEntries.vals()) {
            let migratedUser = migrateUser(user);
            users.put(id, migratedUser);
        };
        for ((email, userId) in emailToUserIdEntries.vals()) {
            emailToUserId.put(email, userId);
        };
        for ((email, otp) in otpEntries.vals()) {
            otpStore.put(email, otp);
        };
        userEntries := [];
        emailToUserIdEntries := [];
        otpEntries := [];
    };
    ...
    public func getOTPCount(email: Email): async Nat {
        // This would need to be implemented with proper rate limiting
        // For now, return 0
        0
    };
