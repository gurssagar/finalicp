...
export default function EnterpriseManagement() {
  ...
  // Mock data for enterprise customers
  const enterprises = Array(7).fill(null).map((_, index) => ({
    id: (index + 1).toString(),
    name: 'Hourglass',
  ...
  const handleDelete = (id: string | number) => {
    console.log('Delete enterprise with id:', id);
    // In a real app, you would make an API call to delete the enterprise
  };
  return <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col">
        <AdminHeader title="Enterprise Management" onSearch={handleSearch} />
        <main className="flex-1 p-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
            <DataTable columns={columns} data={enterprises} onDelete={handleDelete} />
          </div>
        </main>
      </div>
        </div>;
